...
实施顺序建议也应据此调整：先做替代验证，再决定是否启动 [20 界面复原、02 通用会话等实现工单](https://github.com/Leonz3n/PiDock/issues/1)。若保留自研，应围绕 03/04/05/06/07/08 所涉及的跨仓库、服务、浏览器和真实验证能力收窄范围，而非为保留这些能力先把所有外围功能做完。这里没有自动更改工单状态。
