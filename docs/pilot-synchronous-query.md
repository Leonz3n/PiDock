...
状态：用户已接受同步查询方向，以下具体路径由源码核对选定；尚未运行业务服务。试点仓库路径为 `/Users/adber/workspace/shipber/`（用户 2026-09-23 指定，源码链接已按此路径改写并复查，见 [仓库核对](pilot-repository-inspection.md#2026-09-23-重新核对用户指定路径)）。

用户 2026-09-23 另指定了试点环境、对账单与登录方式；supervisor 已在真实环境完成一次诊断性登录与页面探查（登录链路、动态菜单模型、对账单列表与详情入口、本机启动前置条件与安全标记），结果见 [试点环境实测记录](pilot-env-recon-2026-09-23.md)。该探查走的是已部署环境的远程 BFF，**不是本用例的验收证据**：本文的操作验收仍要求查询经本地 BFF、本地 invoice、本地 shipment 并带关联 RPC 日志。
