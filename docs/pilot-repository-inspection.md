...
1. 打开哪条已有对账单（消费流水中的具体记录），以及使用的非生产环境是否即 `http://srb.reconcile.adber.tech`。
2. 由谁完成登录、任务浏览器如何取得已登录状态（凭据只在该浏览器中手动输入，不写入仓库、工单、日志或提交）。
3. 是否批准在真实环境执行本工单：联网、真实登录，并按任务启动本地主壳/BFF/invoice/shipment。
